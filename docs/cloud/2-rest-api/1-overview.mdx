...
Our API is documented using OpenAPI definitions. You can access the [API Reference Page](/reference/api-docs) to check all endpoints and methods that can be used. A important aspect is authentication, that can be check on the [Authentication Guide](./auth)
